...
function render() {
  ...
  $('#average-battery').textContent = `${state.devices.filter(d=>d.battery !== null && d.battery !== undefined).length}/5`;
  $('#devices').innerHTML = state.devices.map(d=>`<button class="device" data-id="${d.id}"><div class="device-top"><span class="status ${esc(d.status)}">${esc(label[d.status]||d.status)}</span><span class="battery">${d.battery == null ? 'Battery unknown' : `${esc(d.battery)}% recorded`}</span></div><h3>${d.id}</h3><span class="model">${esc(d.model)}</span><div class="student">${state.hosted ? 'No student details stored' : esc(d.studentCode ? `Student #${d.studentCode}` : 'Unallocated')}</div><div class="activity">${esc(d.activity||'No activity selected')}</div><div class="activity" style="margin-top:10px">${esc(d.installedApps||'No EDU apps recorded')}</div></button>`).join('');
...
function openDevice(id) { selected=state.devices.find(d=>d.id===id); $('#dialog-model').textContent=selected.model; $('#dialog-title').textContent=selected.id; $('#student-code-label').hidden=state.hosted; $('#student-code').value=selected.studentCode||''; $('#activity').value=selected.activity; $('#installed-apps').value=selected.installedApps||''; $('#battery').value=selected.battery == null ? '' : selected.battery; $('#status').value=selected.status; $('#device-dialog').showModal(); }

$('#save-device').addEventListener('click',async event=>{event.preventDefault();const update={activity:$('#activity').value,installedApps:$('#installed-apps').value,battery:$('#battery').value,status:$('#status').value};if(!state.hosted) update.studentCode=$('#student-code').value;await api(`/api/devices/${selected.id}`,{method:'PATCH',body:JSON.stringify(update)});$('#device-dialog').close();await refresh();});
