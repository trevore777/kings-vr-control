...
function openDevice(id) { selected=state.devices.find(d=>d.id===id); $('#dialog-model').textContent=selected.model; $('#dialog-title').textContent=selected.id; $('#student').value=selected.student; $('#activity').value=selected.activity; $('#installed-apps').value=selected.installedApps||''; $('#battery').value=selected.battery == null ? '' : selected.battery; $('#status').value=selected.status; $('#device-dialog').showModal(); }

$('#save-device').addEventListener('click',async event=>{event.preventDefault();await api(`/api/devices/${selected.id}`,{method:'PATCH',body:JSON.stringify({student:$('#student').value,activity:$('#activity').value,installedApps:$('#installed-apps').value,battery:$('#battery').value,status:$('#status').value})});$('#device-dialog').close();await refresh();});
